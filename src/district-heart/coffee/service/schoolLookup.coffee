    ...
    ($rootScope, $http, $sce) ->
      getSchoolCompanies: (requestData) ->
        ...
        $http
          ...
          data: 'v=1.0&api_key=' + $rootScope.apiKey + '&response_format=json&suppress_response_codes=true&method=getCompaniesByInfo&event_type=' + encodeURIComponent('District Heart Challenge') + '&' + requestData
          headers:
      ...
      getSchoolData: ->
        ...
        requestUrl = '/system/proxy.jsp?__proxyURL=' + encodeURIComponent('https://secure3.convio.net/heartdev/site/SPageServer?pagename=getDistrictHeartSchoolSearchData&pgwrap=n')
        if window.location.href.indexOf(luminateExtend.global.path.secure) is 0
