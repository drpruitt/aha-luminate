  ...
  .factory 'AriaCarouselService', [ 
    ...
    ($timeout)  ->
      init: (elem)->
        ...
        angular.element(elem).find('.owl-item.active a, .owl-prev, .owl-next').attr 'tabindex', '0'
        if angular.element(elem).find('.owl-item.active > a').length is 0
          angular.element(elem).find('.owl-item.active').attr 'tabindex', '0'
        else
          angular.element(elem).find('.owl-item.active a').attr 'tabindex', '0'
