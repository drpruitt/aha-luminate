  ...
  .controller 'CompanyPageCtrl', [
    ...
    ($scope, $rootScope, $location, $filter, $timeout, $uibModal, APP_INFO, TeamraiserCompanyService, TeamraiserTeamService, TeamraiserParticipantService, BoundlessService, TeamraiserRegistrationService, TeamraiserCompanyPageService, PageContentService, $sce) ->
      ...
      $scope.companyPagePhoto1 =
        defaultUrl: APP_INFO.rootPath + 'dist/high-school/image/company-default.jpg'
      
      $scope.editCompanyPhoto1 = ->
        delete $scope.updateCompanyPhoto1Error
        $scope.editCompanyPhoto1Modal = $uibModal.open
          scope: $scope
          templateUrl: APP_INFO.rootPath + 'dist/high-school/html/modal/editCompanyPhoto1.html'
      ...
      window.trPageEdit =
        uploadPhotoError: (response) ->
          ...
          if errorCode is '5'
            window.location = luminateExtend.global.path.secure + 'UserLogin?logout=&NEXTURL=' + encodeURIComponent('TR?fr_id=' + $scope.frId + '&pg=company&company_id=' + $scope.companyId)
